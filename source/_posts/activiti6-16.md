...
我们先来总结下Activiti的各种事件，这些在流程设计器中都可以找到：

事件分类：

| 定义\|事件 | 启动事件（捕获 Catching） | 结束事件(抛出 Throwing) | 边界事件（捕获 Catching） |          中间事件           |
| :--------: | :-----------------------: | :---------------------: | :-----------------------: | :-------------------------: |
|    错误    |   错误（异常）启动事件    |      错误结束事件       |       错误边界事件        |                             |
|    信号    |       信号启动事件        |                         |       信号边界事件        | 定时器中间事件（捕获&抛出） |
|    消息    |       消息启动事件        |                         |       消息边界事件        |    消息中间事件（捕获）     |
|    取消    |                           |      取消结束事件       |       取消边界事件        |                             |
|    补偿    |                           |                         |       补偿边界事件        |      补偿中间抛出事件       |
|   定时器   |       定时启动事件        |                         |       定时边界事件        |   定时器中间事件（捕获）    |
|    终止    |                           |      终止结束事件       |                           |                             |
...
默认信号会在流程引擎范围内进行广播。就是说， 你可以在一个流程实例中抛出一个信号事件，其他不同流程定义的流程实例 都可以监听到这个事件。然而，有时只希望在同一个流程实例中响应这个信号事件。可以使用信号事件定义的scope 属性 （不是BPMN2.0的标准属性）
